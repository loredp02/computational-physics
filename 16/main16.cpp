// PROVARE AD ALLUNGAREU N PO' LA SOLUZIONE
...
#define T_end   10.   // <- fine di evoluzione del sistema di Eq. Diff.
...
int main() {
  ...
  y[1]=0.1; // <- condizioni iniziali del sistema di Eq. Diff.
  T_steps=100;  // <- numero di passi di integrazione
...
double f1(double t, double *v)
{
  ...
  double b = 0.7;
  double gamma = 0.03;
  return -gamma*v[1] - (a - b*cos(t))*sin(v[0]);
}
